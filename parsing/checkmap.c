...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/06/15 02:09:55 by aparedes          #+#    #+#             */
/*   Updated: 2022/06/28 10:35:54 by aparedes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	checkcubextension(char *str, t_long *sl)
{
		...
		&& str[i - 3] == 'b' && str[i - 4] == '.')
	{
		i = open(str, O_RDONLY);
		if (i == -1)
			showerror(sl, "File error");
		else
			close(i);
		return (0);
	}
	showerror(sl, "File .ber is necessary, not founded");
	return (0);
}
